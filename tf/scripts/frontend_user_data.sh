...
aws s3 cp s3://lti-project-code-bucket/frontend.zip /home/ec2-user/frontend.zip || echo "[ADVERTENCIA] No se pudo descargar frontend.zip"
if [ -f /home/ec2-user/frontend.zip ]; then
    unzip /home/ec2-user/frontend.zip -d /home/ec2-user/ || echo "[ADVERTENCIA] Error descomprimiendo frontend.zip"
fi
...
if [ -d /home/ec2-user/frontend ]; then
    ...
    docker build -t lti-frontend . || echo "[ADVERTENCIA] Error construyendo imagen Docker"
    ...
    docker run -d \
        ...
        lti-frontend || echo "[ADVERTENCIA] Error iniciando contenedor"
fi
